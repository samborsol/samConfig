...
void g1_dijetV2Cent_PtSys(){
	...
	for(Long64_t i=0; i<nEvents; i++){
		...
		if(djObj.nJet!=2 || abs(djObj.eta1)>1.8 || djObj.pt1<21 || abs(djObj.eta2)>1.8 || djObj.pt2<16 || djObj.mass<35 || djObj.dphi<2) continue;
		jetplus=jet1a+jet2a;
		...
		for(Long64_t j=i+1; j<nEvents-20; j++){
			...
			if(i==j || djObj.nJet!=2 || abs(djObj.eta1)>1.8 || djObj.pt1<21 || abs(djObj.eta2)>1.8 || djObj.pt2<16 || djObj.mass<35 || djObj.dphi<2) continue;
	...
	for(Long64_t i=0; i<nEvents; i++){
		...
		if(djObjGen.pt1>djObjGen.pt2){
			...
			if( abs(djObjGen.eta1)>1.8 || djObjGen.pt1<21 || abs(djObjGen.eta2)>1.8 || djObjGen.pt2<16 || djObjGen.mass<35 || djObjGen.dphi<2) continue;
	...
	TFile* outputFile = new TFile("objects/roots/ptsys/g1_dijetV2Cent_ptsys.root","RECREATE");
